...
local function OnExplode(inst, target)
    ...
    if target then
	    ...
	    target.components.combat:GetAttacked(inst, DamageConfigTrap_Teeth)
        if METRICS_ENABLED then
			FightStat_TrapSprung(inst,target,DamageConfigTrap_Teeth)
		end
    end
    if inst.components.finiteuses then
	    inst.components.finiteuses:Use(1)
    end
	inst:DoTaskInTime(AutoTimeConfigTrap_Teeth, function() 
		if inst.components.mine then
			...
			if inst.components.mine.onreset then
				inst.components.mine.onreset(inst)
			end
...
local function MakeTeethTrapNormal()
    ...
    -- 堆叠数量
	--[[
    if type(StackNumConfigTrap_Teeth) == "number" and StackNumConfigTrap_Teeth > 1 then
	    inst:AddComponent("stackable")
		inst.components.stackable.maxsize = StackNumConfigTrap_Teeth
	end
	]]
